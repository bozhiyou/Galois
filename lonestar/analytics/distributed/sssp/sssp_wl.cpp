...
using UpdateRequest = Internal::UpdateRequest;
using UpdateRequestIndexer = Internal::UpdateRequestIndexer;
using ReqPushWrap          = Internal::ReqPushWrap;
std::unique_ptr<galois::InsertBag<UpdateRequest>> initBag;

namespace gwl = galois::worklists;
using PSchunk = gwl::PerSocketChunkFIFO<CHUNK_SIZE>;
using OBIM    = gwl::OrderedByIntegerMetric<UpdateRequestIndexer, PSchunk>;

#include "sssp_wl_sync.hh"
...
struct FirstItr_SSSP {
  ...
  FirstItr_SSSP(Graph* _graph, galois::InsertBag<UpdateRequest>* _initBag)
      : graph(_graph), initBag(_initBag) {}

  void static go(Graph& _graph, galois::InsertBag<UpdateRequest>& _initBag) {
    ...
    if (personality == GPU_CUDA) {
      ...
      // one node
      galois::do_all(
          galois::iterate(__begin, __end), FirstItr_SSSP{&_graph, &_initBag},
          galois::no_stats(),
  ...
  void operator()(GNode src) const {
    ...
    for (auto jj : graph->edges(src)) {
      ...
      if (old_dist > new_dist) {
        ...
        initBag->push(UpdateRequest{dst, new_dist});
      }
    }
  }
};
...
struct SSSP {
  ...
  void static go(Graph& _graph, galois::InsertBag<UpdateRequest>& initBag) {
    ...
    [[maybe_unused]] const auto& nodesWithEdges =
...
int main(int argc, char** argv) {
...
#endif
  initBag.reset(new galois::InsertBag<UpdateRequest>());
  ...
  for (auto run = 0; run < numRuns; ++run) {
    ...
    if (execution == Async) {
      SSSP<true>::go(*hg, *initBag);
    } else {
      SSSP<false>::go(*hg, *initBag);
    }
    StatTimer_main.stop();
